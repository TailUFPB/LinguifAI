...
const ChatBot: React.FC = () => {
  ...
  const sendAPIKeyMessage = () => {
    setChatHistory((prevHistory) => [
      ...prevHistory,
      {
        text: "Olá! Eu sou o LinguiTalk, um chatbot para lhe auxiliar na exploração dos seus dados! Primeiro, insira uma chave API válida do ChatGPT:",
        origin: "bot",
      },
    ]);
  };
  ...
  const handleApiKeySubmit = async () => {
    ...
    const response = await axios.post(getApiUrl("apikey"), {
      apikey: apiKeyInput,
    });
    const botResponse: Message = { text: response.data.reply, origin: "bot" };
    setChatHistory((prevHistory) => [...prevHistory, botResponse]);
            ...
            <div>
              {chatHistory.map((msg, index) => (
                ...
                >
                  <p
                    className={`text-sm ${
                      msg.origin === "user" ? "text-blue-700" : "text-red-700"
                    }`}
                  >
