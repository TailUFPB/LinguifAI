...
export default function HomeView() {
  ...
  useEffect(() => {
    const fetchClassifiers = async () => {
      while (true) {
        try {
          const response = await axios.get(getApiUrl("get-classifiers"));
  ...
  useEffect(() => {
    const sendSelectedFileToBackend = async () => {
      if (selectedFile) {
        ...
        try {
          await axios.post(getApiUrl("receive"), formData, {
            headers: {
  ...
  const handleChangeSelectedClassifier = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
