...
export default function TrainView() {
  ...
  const handleChangeSelectedLabel = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
  ...
  const handleCancelTraining = async () => {
    ...
    try {
      await axios.post(getApiUrl("cancel-training"));
  ...
  const handleRnnSubmit = async () => {
    ...
    const url = getApiUrl("neural-network-rnn");
  ...
  const handleNbSubmit = async () => {
    ...
    const url = getApiUrl("neural-network-nb");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(getApiUrl("training-status"));
        const {
          training_progress,
            ...
            <div className="relative w-full">
              ...
              {!isLoading && (
                <button
                  ...
                  onClick={
                    trainingType === "nb" ? handleNbSubmit : handleRnnSubmit
                  }
                  disabled={isLoading}
                >
              ...
              {hasTrained && train_losses.length > 0 && (
                <div className="bg-gray-50 rounded-lg pt-4 my-4 text-black">
                  <ReactApexChart
                    options={ReactApexChartsDefaultOptions}
                    series={[
                      {
                        name: "Treino",
                        data: train_losses,
